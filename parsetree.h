...
public:
    ...
    Val Eval(map<string,Val>& symbols) {
        ...
        if (expression.isInt())
        {
            if(expression.ValInt() != 0)
            {
                auto answer = right->Eval(symbols);
                cout << answer;
                Eval(symbols);
            }
            return Val();
...
public:
    ...
    Val Eval(map<string,Val>& symbols) {
        ...
        auto answer = L + R;
        if( answer.isErr() )
            runtime_err(GetLineNumber(), answer.GetErrMsg());
...
public:
    ...
    Val Eval(map<string,Val>& symbols) {
        auto L = left->Eval(symbols);
        //if(left->left) { return Val(L);}       //need to be fixed
        if( L.isErr() )
            runtime_err(GetLineNumber(), L.GetErrMsg());
